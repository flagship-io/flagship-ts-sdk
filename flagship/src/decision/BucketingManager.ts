...
import { CampaignDTO, primitive, Targetings, ThirdPartySegment, TroubleshootingLabel, VariationDTO, VariationGroupDTO } from '../types'
import { VisitorAbstract } from '../visitor/VisitorAbstract'
import { Segment } from '../hit/index'
import { Troubleshooting } from '../hit/Troubleshooting'
import { ALLOCATION, BUCKETING_NEW_ALLOCATION, BUCKETING_VARIATION_CACHE, GET_THIRD_PARTY_SEGMENT, LogLevel, THIRD_PARTY_SEGMENT_URL } from '../enum/index'
...
export class BucketingManager implements IDecisionManager {
  ...
  async getCampaignsAsync (visitor: VisitorAbstract): Promise<CampaignDTO[]> {
    ...
    bucketingContent.campaigns.forEach(campaign => {
      const currentCampaigns = this.getCurrentCampaigns(campaign.variationGroups, campaign.id, campaign.type, visitor)
      if (currentCampaigns) {
  ...
  private getCurrentCampaigns (variationGroups: VariationGroupDTO[], campaignId: string, campaignType: string, visitor: VisitorAbstract): CampaignDTO | null {
