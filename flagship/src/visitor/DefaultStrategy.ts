...
export class DefaultStrategy extends StrategyAbstract {
  ...
  protected async GetCampaignsFromDecisionManager (functionName: string, now: number) {
  ...
  protected sendFetchErrorTroubleshooting ({
    error, logData, now, functionName, campaigns
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  }:{ error: any, logData: Record<string, unknown>, now:number, functionName: string, campaigns: CampaignDTO[] | null }) {
  ...
  async fetchFlags (): Promise<void> {
    ...
    let campaigns: CampaignDTO[] | null = null
    const data = await this.GetCampaignsFromDecisionManager(functionName, now)
    if (!data) {
      return
    }
    try {
      ...
      this.visitor.emit(EMIT_READY, data.error)
      ...
      this.sendFetchErrorTroubleshooting({ error, logData, campaigns, now, functionName })
    }
  }
