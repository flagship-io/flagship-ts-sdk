...
export class FsHeader extends LitElement {
  ...
  protected _onClickCloseCard () {
    this.dispatchEvent(
      new CustomEvent('onFsClickCloseCard', { bubbles: true, composed: true })
    )
  }

  protected _onValidation () {
    this.dispatchEvent(
      new CustomEvent('onFsValidation', { bubbles: true, composed: true })
    )
  }
    ...
    return html`
      <div class="card-header card-header-d-flex">
        ...
        <div class="btn-container">
          <div class="validation-btn" @click=${this._onValidation}>
